...
proc dump_header { LOG output_log } {
  puts $LOG "/********************************************"
...
proc vw_dvc_go2uvm {LOG top_mod } {
...
#  set out_ports [find object -out -list -path /$top_mod *]
  ...
  set in_ports [find -ports * -inputs *]
  set out_ports [find -ports * -outputs *]
  set inout_ports [find -ports * -inout *]
  ...
  foreach item $in_ports {
    ...
    set tmp_s [split $port_des \[]
    set tmp_s_size [llength $tmp_s]
    #puts $port_des
    # puts "split: $tmp_s"
    #puts $tmp_s_size
    if {$tmp_s_size ==  1} {
      #puts "$item size is 1"
      set tmp_port_size {[0:0]}
    } else {
      #puts "$item size is more than 1"
      set tmp1_s [lindex $tmp_s 1]
      set tmp2_s [split $tmp1_s \]]
      set tmp3_s [lindex $tmp2_s 0]
      set tmp4_s [split $tmp3_s :]
      set msb_val [lindex $tmp4_s 0]
      set lsb_val [lindex $tmp4_s 1]
      #puts "split: 1st: $tmp1_s "
      #puts "split: 2: $tmp2_s "
      #puts "split: 3: $tmp3_s "
      #puts "split: 4: $msb_val $lsb_val "
      set tmp_port_size "[$msb_val:$lsb_val]"
    }
    set tmp_port_name $item
    lappend in_port_names_l $tmp_port_name
    # echo "SV: $tmp_port_size"
    lappend in_port_sizes_l $tmp_port_size
    #puts "IN: $tmp_port_name $tmp_port_size"
  }
  # Create OUTPUT port name & size
  set out_port_names_l []
  ...
  foreach item $inout_ports {
    ...
    set tmp_port_name $item
    lappend inout_port_names_l $tmp_port_name
  ...
  set num_in_ports [llength $in_port_names_l]
  set cur_port_num 0
  while { $cur_port_num < $num_in_ports } { 
    set port_name [lindex $in_port_names_l $cur_port_num]
  ...
  set num_out_ports [llength $out_port_names_l]
  set cur_port_num 0
  while { $cur_port_num < $num_out_ports } { 
    set port_name [lindex $out_port_names_l $cur_port_num]
  ...
  set num_inout_ports [llength $inout_port_names_l]
  set cur_port_num 0
  while { $cur_port_num < $num_inout_ports } { 
    set port_name [lindex $inout_port_names_l $cur_port_num]
...
proc test_go2uvm { LOG top_mod } {
  ...
  set class_hdr [format "%s_if %s_if_0 (.*);" $top_mod $top_mod]
  puts $LOG "  $class_hdr"
  puts $LOG ""
  puts $LOG "  // Connect TB clk to Interface instance clk"
  puts $LOG ""
  puts $LOG "  // DUT instance"
  set class_hdr [format "%s %s_0 ();" $top_mod $top_mod]
  puts $LOG "  $class_hdr"
  puts $LOG ""
  puts $LOG ""
  puts $LOG "  // Using VW_Go2UVM"
  set class_hdr [format "%s_test %s_test_0;" $top_mod $top_mod]
  puts $LOG "  $class_hdr" 
  puts $LOG "  initial begin : go2uvm_test"
  set class_hdr [format "  %s_test_0 = new ();" $top_mod]
  puts $LOG "  $class_hdr" 
  puts $LOG "    // Connect virtual interface to physical interface"
  set class_hdr [format "%s_test_0.vif = %s_if_0;" $top_mod $top_mod]
  puts $LOG "    $class_hdr"
  puts $LOG "    // Kick start standard UVM phasing"
  puts $LOG "    run_test ();"
  puts $LOG "  end : go2uvm_test"
  set class_hdr [format "endmodule : %s_go2uvm" $top_mod]
  puts $LOG "$class_hdr"
  puts $LOG ""
}
...
proc vw_go2uvm_macros { LOG } {
  ...
  puts $LOG ""
  puts $LOG "`define vw_uvm_info(ID,MSG,VERBOSITY) \\"
...
puts $log "    phase.raise_objection (`VW_KW_1067854538);"
puts $log "    `vw_uvm_info (log_id, \"Driving stimulus via UVM\", UVM_MEDIUM)"
puts $log "    `VW_KW_1067854538.main ();"
puts $log "    `vw_uvm_info (log_id, \"End of stimulus\", UVM_MEDIUM)"
puts $log "    phase.drop_objection (`VW_KW_1067854538);"
...
puts $log ""
puts $log ""
puts $log "  `vw_uvm_info(log_id, \$sformatf(\"RELNOTES \\n%s\", vw_rel_str), UVM_NONE)"
puts $log ""
...
puts $log "    `vw_uvm_info (log_id, \$sformatf
...
proc vw_go2uvm_version { LOG } {
  ...
  puts $LOG " // |   `uvm_info (\"Go2UVM\", get_ver, UVM_MEDIUM);"
...
proc g2u_flist { LOG top_mod} {
  ...
  puts $LOG ""
  puts $LOG ""
  set flist_hdr [format "../go2uvm/%s_if.sv" $top_mod]
  puts $LOG "$flist_hdr"
...
proc go2uvm { top_mod } {
  ...
  # Go2uvm file automation
  exec mkdir ../go2uvm
  set op_log_fname [format "../go2uvm/%s_if.sv" $top_mod]
  set op_g2u_fname [format "../go2uvm/%s_go2uvm.sv" $top_mod]
  set G2U [open $op_g2u_fname "w"] 
  set LOG [open $op_log_fname "w"]

  dump_header $LOG $op_log_fname
  dump_header $G2U $op_g2u_fname

  vw_dvc_go2uvm $LOG $top_mod
  test_go2uvm $G2U $top_mod

  close $G2U
  close $LOG 

  # Go2uvm package library
...
# Newly added line 
  ...
  puts "Successfully generated Go2UVM TB & Test along with GO2UVM library installation for module: $top_mod "
